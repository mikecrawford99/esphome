...
void Madoka::control(const ClimateCall &call) {
  ...
  auto setpoint_cmd = setpoint_adapter_.buildSetpointCommand(call);
  if (!setpoint_cmd.empty()) {
    this->query_(CMD_SET_SETPOINT, setpoint_cmd, 400);
  }
      ...
      while (i < message_size) {
        ...
        switch (argument_id) {
          case 0x20: {
            ...
            setpoint_adapter_.handleHighTemperature(temp_high);
            break;
          }
          case 0x21: {
            std::vector<uint8_t> val(msg.begin() + i, msg.begin() + i + len);
            float temp_low = (float) (val[0] << 8 | val[1]) / 128;
            setpoint_adapter_.handleLowTemperature(temp_low);
            break;
          }
        }
        i += len;
